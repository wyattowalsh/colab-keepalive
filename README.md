...
- Provides optional local idle-signal controls such as jittered intervals, screen wake lock, and document-level activity events.
...
- Provides a polished popup with enable/disable, interval and jitter controls, live status, uptime, local-signal indicators, theme toggle (light/dark/auto), browser notification settings, keyboard shortcut hints, test click, and reset controls.
...
- `notifications`: shows optional browser alerts for errors and idle dialogs.
- `contextMenus`: exposes Colab-only right-click actions for toggle, click-now, and settings.
...
### Optional Local Idle Signals

When local idle-signal controls are enabled, the content script can add optional browser-side signals on Colab pages:

- **Jittered intervals**: the actual tick interval varies randomly around the configured base by the jitter percentage (default 15%). This avoids perfectly periodic local timing.
- **Screen Wake Lock**: requests `navigator.wakeLock` to reduce display sleep while the page is active, when the browser supports it.
- **Document-level activity events**: dispatches bounded `mousemove`, `mousedown`, `mouseup`, `keydown`, `keyup`, `scroll`, `wheel`, and `focus` events at the document level without reading notebook contents.
...
- **Toggle humanize** — enable or disable optional local idle-signal features (wake lock, activity events, jitter).
...
- **Local-signal indicators are off**: the content script may not have requested wake lock yet (it tries on the next tick), or the browser does not support the Wake Lock API. Activity events run only when the tab is active.
- **Runtime still disconnects**: Colab server-side runtime limits, quotas, idle policies, account restrictions, and abuse protections still apply. Local idle-signal controls cannot override server-side limits.
...
- Confirm permissions are limited to `storage`, `alarms`, `notifications`, `contextMenus`, and Colab host permissions.

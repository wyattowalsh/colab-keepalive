...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  void handleRuntimeMessage(message, sender)
    ...
    .catch((error) => {
      ...
      sendResponse(errorResponse("UNHANDLED_ERROR", error?.message || "Unexpected error"));
    });
  return true;
});
...
async function handleRuntimeMessage(message, sender) {
  ...
  if ((type === "CKA_STATUS_UPDATE" || type === "CKA_ERROR") && !validateSender(sender)) {
    return errorResponse("INVALID_SENDER", "Rejected message from non-Colab sender");
  }
  ...
  switch (type) {
    ...
    case "CKA_APPLY_SETTINGS":
      if (hasTabSender && !validateSender(sender)) {
        return errorResponse("INVALID_SENDER", "Rejected settings request from non-Colab sender");
      }
...
function validateSender(sender) {
  ...
  try {
    ...
    return origin === "https://colab.research.google.com" || origin.endsWith(".colab.research.google.com");
  } catch {
    return false;
  }
}
...
async function setDefaultSettings() {
  ...
  const needsWrite = Object.entries(merged).some(([key, value]) => stored[key] !== value);
  if (needsWrite) {
...
async function applySettingsFromMessage(payload) {
  const nextSettings = validateSettings({ ...(await getSettings()), ...(payload?.settings || {}) });
...
async function handleSyncStorageChanged(changes) {
  const settingChanged = Object.keys(changes).some((key) => key in DEFAULT_SETTINGS);
  if (!settingChanged) {
    return;
  }
...
async function queryColabTabs() {
  const tabSets = await Promise.all(COLAB_URL_PATTERNS.map((url) => chrome.tabs.query({ url })));
  const seen = new Set();
  return tabSets
    .flat()
    .filter((tab) => typeof tab.id === "number" && !seen.has(tab.id) && seen.add(tab.id));
}
...
async function sendToTabSafely(tabId, message) {
  try {
    ...
    return errorResponse("TAB_MESSAGE_FAILED", error?.message || "Could not message Colab tab");
  }
}
...
async function testClickOpenColabTab() {
  const tabs = await queryColabTabs();
  if (tabs.length === 0) {
    return errorResponse("NO_COLAB_TABS", "No open Google Colab tabs were found");
  }
...
async function recordTabStatus(tabId, status = {}) {
  const { [SESSION_STATUS_KEY]: statuses = {} } = await chrome.storage.session.get(SESSION_STATUS_KEY);
...
async function updateBadge(settings) {
  ...
  const tabs = await queryColabTabs();
  const { [SESSION_STATUS_KEY]: statuses = {} } = await chrome.storage.session.get(SESSION_STATUS_KEY);
  const hasError = Object.values(statuses).some((status) => status?.state === "error" || status?.warning);
  ...
  await Promise.all(
    tabs.map((tab) => {
      ...
      const state = status?.state === "error" || status?.warning ? "ERR" : currentSettings.enabled ? "ON" : "OFF";
...
async function setBadgeForTab(tabId, state) {
  ...
  const color = state === "ON" ? "#1a7f37" : state === "ERR" ? "#d1242f" : "#6e7781";
...
async function getPopupStatus() {
  ...
  const liveStatuses = tabs.map((tab) => statuses[String(tab.id)] || { tabId: tab.id, state: "unknown" });
  return {
    settings,
...
function aggregateStatus(statuses) {
  const failureCount = statuses.reduce((sum, status) => sum + Number(status.failureCount || 0), 0);
  const lastClickAt = statuses
  ...
  const state = statuses.some((status) => status.state === "error")
    ...
    : statuses.some((status) => status.warning)
      ...
      : statuses.length > 0
        ? "ready"
        : "no-tabs";
  return { state, failureCount, lastClickAt };
}
...
async function ensureReconciliationAlarm() {
  ...
  if (!alarm) {
    await chrome.alarms.create(ALARM_NAME, { periodInMinutes: RECONCILE_INTERVAL_MINUTES });
  }
}
...
function isAllowedColabUrl(value) {
  try {
    ...
    return url.protocol === "https:" && (
      COLAB_ORIGINS.has(url.origin) || url.hostname.endsWith(".colab.research.google.com")
    );
  } catch {
    return false;
  }
